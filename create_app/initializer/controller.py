...
''',
    ...
    def _write_mcp_scaffold(self):
        """Write the opinionated MCP hub skeleton for the MCP project type."""
        if self.fw != "mcp":
            return
        files = {
            "registry.json": {
                "version": 1,
                "project": self.p_name,
                "tools": [{"name": "example_tool", "path": "mcp-tools/_template", "language": "python", "enabled": False}],
            },
            "config/mcp.config.json": {"mcpServers": {"example-tool": {"command": "python", "args": ["mcp-tools/_template/server.py"], "env": {}}}},
            "mcp-tools/_template/README.md": "# MCP Tool Template\n\nCopy this directory to create a new tool, then update `metadata.json`.\n",
            "mcp-tools/_template/metadata.json": {
                "name": "example_tool", "display_name": "Example Tool", "description": "Replace with your tool description.",
                "category": "Utilities", "language": "python", "mcp_compatible": True,
                "parameters": {"type": "object", "properties": {"query": {"type": "string"}}, "required": ["query"]},
                "env_vars": [],
            },
            "mcp-tools/_template/requirements.txt": "mcp[cli]\n",
            "mcp-tools/_template/.env.example": "# Add tool-specific secrets here\n",
            "mcp-tools/_template/tool.py": "from typing import Any\n\ndef run_tool(query: str, **kwargs: Any) -> dict:\n    if not query:\n        return {'success': False, 'error': 'query is required'}\n    return {'success': True, 'data': {'query': query}}\n",
            "mcp-tools/_template/server.py": "from mcp.server.fastmcp import FastMCP\nfrom tool import run_tool\n\nmcp = FastMCP('example_tool')\n\n@mcp.tool()\ndef example_tool(query: str) -> dict:\n    return run_tool(query)\n\nif __name__ == '__main__':\n    mcp.run()\n",
            "mcp-tools/_template/examples/basic_usage.py": "from tool import run_tool\nprint(run_tool('hello'))\n",
            "mcp-tools/_template/tests/test_tool.py": "from tool import run_tool\n\ndef test_tool():\n    assert run_tool('hello')['success'] is True\n",
            "scripts/generate_registry.py": "# Regenerate registry.json from mcp-tools metadata.\nimport json\nfrom pathlib import Path\n\nroot = Path(__file__).resolve().parents[1]\ntools = []\nfor metadata in sorted((root / 'mcp-tools').glob('*/metadata.json')):\n    data = json.loads(metadata.read_text())\n    tools.append({'name': data['name'], 'path': str(metadata.parent.relative_to(root)).replace('\\\\', '/'), 'language': data.get('language', 'python'), 'enabled': metadata.parent.name != '_template'})\n(root / 'registry.json').write_text(json.dumps({'version': 1, 'project': root.name, 'tools': tools}, indent=2) + '\\n')\n",
            "docs/MCP_PROJECT_SKELETON.md": "# MCP Project Skeleton\n\nCopy `mcp-tools/_template` to create a new MCP tool. Update its README and metadata, implement `tool.py`, expose it through `server.py`, add tests, then run `python scripts/generate_registry.py`.\n",
            "README.md": f"# {self.p_name}\n\nMCP tool hub generated by init-app. Copy `mcp-tools/_template` to start a new tool, update `registry.json`, and run `python scripts/generate_registry.py`.\n\nRun a tool with `python mcp-tools/_template/server.py`.\n",
        }
